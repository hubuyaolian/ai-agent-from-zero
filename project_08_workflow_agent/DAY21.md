...
业务流程 Agent 的压力不只来自用户请求数，还来自工具调用放大。一个用户请求可能拆成 5 个步骤，每个步骤失败后又重试 3 次；如果 20 个用户同时触发，就可能变成几百次工具调用。

压力放大的典型来源：

| 来源 | 风险 | 治理手段 |
|------|------|----------|
| LLM 规划过细 | 一个任务拆成过多工具调用 | 限制最大步骤数、计划审核 |
| 并发用户增加 | 同一工具被集中调用 | 按用户/租户/工具限流 |
| 重试风暴 | 上游过载时越重试越糟 | 指数退避、jitter、重试预算 |
| 非幂等副作用 | 重复发通知、重复归档 | 幂等键、非幂等禁自动重试 |
| 审批积压 | 大量敏感步骤等待人审 | 队列、超时、批量审批、升级提醒 |
| 工具连续失败 | 每个流程都撞同一个故障服务 | 熔断、降级、暂停调度 |

限流解决“单位时间内不要打爆工具”，排队解决“暂时处理不过来但还能等”，熔断解决“服务明显坏了就不要继续撞”。三者不要混用：
...
压测流程 Agent 时，至少记录这些指标：

| 指标 | 说明 |
|------|------|
| 任务成功率 | 一次业务指令最终成功的比例 |
| 平均步骤数 | LLM planner 是否过度拆解 |
| 工具调用数/任务 | 一个任务放大成多少工具请求 |
| P95/P99 任务耗时 | 用户从提交到完成要等多久 |
| 重试率 / 重试耗尽率 | 是否发生上游抖动或参数问题 |
| 限流命中率 | 是否有工具被集中打爆 |
| 熔断次数 | 哪些工具连续失败 |
| 审批队列长度 | 敏感任务是否积压 |
| 成本/任务 | token、工具服务、人工审批成本 |

仓库中提供了一个可选示例：`project_08_workflow_agent/examples/agent_pressure_governance_demo.py`。它演示滑动窗口限流和熔断器如何包在工具调用之前。教学版不直接改主 `WorkflowEngine`，是为了先把“压力治理应该发生在工具副作用之前”这条边界讲清楚。
...
6. **压力治理实验**：运行或阅读 `examples/agent_pressure_governance_demo.py`，解释为什么限流应该按用户、租户或工具维度设计，而不是全局一个计数器。

7. **流程压测方案**：为“读取销售数据 → 生成日报 → 发送通知”设计压测方案，包含并发用户数、任务成功率、P95/P99、重试率、审批队列长度和成本/任务。

8. **Flake8 自检**：确保代码通过 `flake8 project_08_workflow_agent/` 的检查。
