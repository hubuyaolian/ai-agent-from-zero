...
开发类 Agent 的压测不能只看“每秒多少请求”。一个请求可能持续几分钟，期间会多次规划、写代码、跑测试、修复、写文档和等待审批。所以更合理的单位是“每个任务占用多少资源，以及系统能同时承载多少个长程任务”。

需要重点观察：

| 指标 | 含义 |
|------|------|
| 任务完成率 | 输入一个需求后，最终成功交付的比例 |
| 平均运行时间 / P95 运行时间 | 一个任务占用 runner 多久 |
| 平均工具调用数 | 是否出现工具调用膨胀 |
| 平均修复轮次 | Developer-Tester 循环是否失控 |
| 沙箱 CPU/内存/磁盘峰值 | 生成项目测试是否压垮执行环境 |
| 队列等待时间 | 多个开发任务同时提交时等待多久 |
| 人工介入率 | 需要人审、diff review、最终确认的比例 |
| 失败归因分布 | 规划失败、代码失败、测试失败、路径违规、超时分别占比 |
| 成本/任务 | token、工具调用、沙箱运行和人工审批成本 |

长程 Agent 压测的一种简单流程：

1. 准备 20-50 个固定需求：todo、calculator、notes、文件处理、错误输入、越权路径。
2. 逐步增加并发任务数：1、2、5、10。
3. 每个任务设置最大运行时间、最大修复轮次、最大工具调用数。
4. 记录成功率、P95 运行时间、失败原因、成本和人工介入率。
5. 当失败率或队列等待明显上升时，停止加压，定位瓶颈是 Planner、Tester、沙箱、文件写入还是审计日志。

治理手段和 P08 类似，但粒度更长：

- 用任务队列限制同时运行的开发任务数。
- 用每个 run 的预算限制 token、工具调用数和修复轮次。
- 用 sandbox 资源限制 CPU、内存、磁盘和网络。
- 用 checkpoint 让等待审批的任务不占用 runner。
- 用熔断机制暂停连续失败的工具或模板。
- 用 trace 把失败归因到具体 Agent、工具和阶段。

### 5. Sandbox 是长程开发 Agent 的底线
...
5. 把测试运行迁移到容器沙箱。
6. 为每类需求建立基准任务集，评估成功率、修复轮次、成本、P95 运行时间、队列等待和人工介入率。
