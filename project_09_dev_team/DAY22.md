...
> 技术状态说明（2026）：多 Agent 开发团队并不落后，LangGraph、OpenAI Agents SDK、AutoGen、CrewAI 都仍在围绕 subagents、handoffs、teams、flows 等模式演进。MCP 正在把工具和上下文接入协议化，Google ADK、PydanticAI 等框架也在强化类型安全、部署、观测和评估能力。但当前最佳实践已经从“让多个 Agent 全自动写完项目”转向“代码编排 + 专家 Agent + 结构化产物 + 沙箱执行 + 人工审批 + 可观测评估”。因此本课不能承诺无边界全自动交付生产代码，而应把目标设为**受控的本地开发闭环**。
...
- 理解 MCP 在多 Agent 工具共享中的作用，以及 ADK、PydanticAI、Agents SDK 等框架的取舍。
