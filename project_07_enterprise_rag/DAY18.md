...
> 技术状态说明（2026）：向量检索 + BM25 + RRF 融合 + Reranker 仍是主流 RAG 检索路线，不落后。但具体模型和基础设施要更新：新建 Qwen 检索项目优先考虑 `text-embedding-v4` 或 Qwen3 Embedding/Reranker；`text-embedding-v3` 更适合兼容已有 v3 索引。ChromaDB、SQLite、pickle 在本课中作为本地教学实现，生产环境通常会换成托管向量检索、全文检索、对象存储和权限系统。2025-2026 年还需要补充三类扩展视角：MCP 让数据源和工具接入协议化，GraphRAG 适合复杂关系型知识，Agentic RAG 则把检索放进 Agent 的规划与验证循环中。
...
6. [第五部分：2026 前沿扩展：MCP、GraphRAG 与 Agentic RAG](#第五部分2026-前沿扩展mcpgraphrag-与-agentic-rag)
7. [核心原理深度解析](#核心原理深度解析)
8. [课后练习](#课后练习)
...
- 理解 MCP、GraphRAG、Agentic RAG 与本课传统企业 RAG pipeline 的关系和边界。
...
## 第五部分：2026 前沿扩展：MCP、GraphRAG 与 Agentic RAG

本课实现的是一条受控的企业 RAG pipeline：文档摄取、分块、索引、检索、重排、生成、引用校验和评估。它仍然是生产 RAG 的基本功。但 2025-2026 年的 RAG 系统正在往三个方向扩展，学习者需要知道它们解决的是哪一层问题，而不是把它们混成同一种技术。

### 1. MCP：把工具和数据源接入标准化

MCP（Model Context Protocol）不是一种新的检索算法，而是让 AI 应用以统一协议访问工具、数据源和上下文的连接层。传统 Function Calling 通常是“当前应用自己定义工具 schema 并自己执行工具”；MCP 则更像把工具和数据源封装成可复用的 server，Agent 或应用作为 client 连接它们。

在企业 RAG 场景中，MCP 可以用于：

- 把文档库、工单系统、CRM、代码仓库、数据仓库封装为统一的上下文服务。
- 让多个 Agent 或多个应用复用同一组检索和工具能力。
- 通过 tool filtering、审批策略和 tracing 控制 Agent 能看到哪些能力、调用了哪些能力。

但 MCP 不能替代本课中的权限过滤、PII 脱敏、引用校验和评估。协议统一只解决“怎么接入”，不自动保证“接入后安全可信”。

### 2. GraphRAG：补足复杂关系型知识

传统向量 RAG 擅长“找到语义相近的段落”，BM25 擅长“找到精确词命中的段落”。但当问题依赖实体关系、跨文档归纳或组织结构时，仅返回 Top-K chunk 可能不够。例如：

- “哪些供应商同时影响产品 A 和产品 B 的交付风险？”
- “这几条制度之间是否存在审批链路冲突？”
- “某客户投诉背后涉及哪些部门、系统和历史变更？”

GraphRAG 的核心思路是先抽取实体、关系和社区结构，构建知识图谱索引，再结合图遍历、社区摘要或实体关系检索回答问题。它适合关系密集、跨文档、多跳推理的场景。

引入 GraphRAG 的代价也更高：实体抽取质量、关系 schema、图谱更新、社区摘要、图数据库查询、权限传播都会成为新的工程问题。对于普通制度问答、单文档事实查询，混合检索 + Reranker 往往已经足够。

### 3. Agentic RAG：让检索进入 Agent 决策循环

本课 pipeline 的检索策略是预设的：改写查询、向量 + BM25、RRF、Rerank、质量不够就扩大 top-k 或 query variants。Agentic RAG 的区别是：Agent 可以根据问题复杂度自主决定是否检索、检索哪个数据源、拆成几个子问题、是否二次检索、是否需要验证证据。

典型能力包括：

- 查询分解：把复杂问题拆成多个可检索子问题。
- 策略选择：在向量检索、关键词检索、SQL 查询、图检索、网页检索之间选择。
- 迭代验证：回答前检查证据是否足够，不够就继续检索或拒答。
- 多源协调：同时处理文档、结构化数据、工单、代码等异构来源。

Agentic RAG 的风险是成本、延迟和可控性。Agent 多走一步就多一次工具调用、多一次状态记录、多一次安全面。生产环境必须给它加最大步数、最大成本、工具白名单、审计 trace、失败降级和人工介入点。

### 4. 如何和本项目衔接

本项目不直接实现 MCP、GraphRAG 或 Agentic RAG，因为两天项目的重点是把企业 RAG 的基础闭环跑通。合理的扩展路线是：

仓库中提供了一个纯内存的可选示例：`project_07_enterprise_rag/examples/agentic_rag_demo.py`。它不接真实 LLM 或向量库，只演示“查询拆解 → 多轮检索 → 证据不足拒答 → 最大步数约束”的 Agentic RAG 控制边界。

| 扩展方向 | 可接入位置 | 必须补的治理能力 |
|----------|------------|------------------|
| MCP | `DocumentLoaderFactory`、`HybridRetriever`、外部工具调用层 | 工具过滤、审批、调用审计、凭证隔离 |
| GraphRAG | `retrieval/` 新增 graph retriever，与向量/BM25 一起融合 | 实体级 ACL、图谱更新、关系证据引用 |
| Agentic RAG | `graph/workflow.py` 中新增 planner / retrieval decision 节点 | 最大步数、成本预算、trace、失败降级 |

学习顺序上不要反过来：先掌握本课的可控 pipeline，再讨论 GraphRAG 和 Agentic RAG。否则容易只学到“让 Agent 自己决定”，却没有可验证、可审计、可回滚的工程边界。
