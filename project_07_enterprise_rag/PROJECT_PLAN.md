...
- 掌握 RAG 延迟、吞吐量、瓶颈阶段和压测指标的基本分析方法。
...
| 19.8 | 吞吐量示例 | `examples/rag_throughput_demo.py` | 能估算阶段吞吐、端到端延迟和瓶颈 |
...
| 性能基线 | 能记录 P95/P99、错误率、召回率、成本/请求 |
...
- Day 19 解释清楚 RAG 吞吐量与单次延迟的区别，以及如何识别瓶颈阶段。
- 明确标注教学版实现与生产环境差异。
...
| 吞吐瓶颈误判 | 只优化向量库但整体 QPS 不提升 | 用阶段吞吐估算找最小容量阶段 |
| 压测只看平均值 | 尾部延迟和错误率被掩盖 | 记录 P95/P99、错误率、队列等待和成本 |
...
6. 建立压测基线：固定问题集、并发档位、P95/P99、错误率、召回率、引用准确率和成本指标。
7. 增加人工反馈闭环：用户可标记回答是否有用、引用是否准确。
8. 把外部数据源接入抽象为 MCP Server 或企业连接器，并保留权限、审计和凭证隔离。
9. 对复杂关系型知识评估 GraphRAG，但需要同步设计实体级 ACL、图谱更新和关系证据引用。
10. 对复杂问题引入 Agentic RAG planner 时，必须设置最大步数、成本预算、工具白名单和失败降级。
