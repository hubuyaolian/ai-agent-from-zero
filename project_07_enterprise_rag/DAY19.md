...
延迟（latency）回答的是“一个用户等多久”，吞吐量（throughput / QPS）回答的是“系统一秒能稳定处理多少请求”。企业 RAG 不能只看单次 demo 是否跑通，还要看高峰期能不能稳定服务。

一个简化估算公式：

```
单阶段吞吐 QPS ≈ 并发槽位数 / 单次阶段耗时（秒）
整条 RAG 流水线吞吐 ≈ 所有阶段吞吐的最小值
```
...
| 阶段 | 单次耗时 | 并发槽位 | 理论吞吐 |
|------|----------|----------|----------|
| 向量检索 | 60ms | 32 | 约 533 QPS |
| BM25 检索 | 40ms | 32 | 约 800 QPS |
| Reranker | 300ms | 8 | 约 26 QPS |
| 回答生成 | 4s | 4 | 约 1 QPS |
| 质量检查 | 1s | 8 | 约 8 QPS |

这条链路的瓶颈不是向量库，而是回答生成。即使向量检索很快，只要回答生成阶段只有 4 个并发槽位、每次 4 秒，系统稳定吞吐就很难超过 1 QPS。
...
查询改写
  ↓
向量检索 ─┐
          ├─ RRF 融合 → Rerank → 生成 → 引用校验 → 质量检查
BM25 检索 ┘
```

向量检索和 BM25 可以并行，所以单次延迟取两者中较慢的一个，而不是简单相加。但它们依然分别消耗检索服务资源，高并发下都要看连接池、CPU、索引缓存和 metadata filter 的成本。

仓库中提供了一个可选示例：`project_07_enterprise_rag/examples/rag_throughput_demo.py`。它用纯 Python profile 演示如何估算阶段延迟、整条流水线吞吐和瓶颈阶段。这个示例不压真实模型，只用于教学中理解容量估算方法。
...
|------|------|
| QPS / RPS | 每秒完成多少问答请求 |
| P50 / P95 / P99 延迟 | 大多数用户和尾部用户分别等多久 |
| Error Rate | 429、超时、模型错误、检索失败比例 |
| Retrieval Recall@K | 高压力下召回质量是否下降 |
| Citation Accuracy | 高压力下引用是否仍然支撑答案 |
| Token/s 与成本/请求 | 模型吞吐和费用是否可接受 |
| Cache Hit Rate | 查询改写、检索、rerank、答案缓存命中率 |
| Queue Wait Time | 请求排队等待多久 |
| ACL Filter Drop Rate | 权限过滤丢弃了多少候选，是否导致召回不足 |

典型压测流程：

1. 固定一组代表性问题：事实问答、多轮追问、无答案问题、权限隔离问题。
2. 从低并发开始逐步升高：1、5、10、20、50 个并发用户。
3. 每一档至少跑几分钟，记录 P95/P99、错误率、召回率和成本。
4. 找到第一个明显退化点：延迟飙升、错误率上升、引用质量下降或队列积压。
5. 针对瓶颈做优化：缓存、批量 rerank、增加模型并发、缩短上下文、异步审计、降级质量检查。

RAG 的压测还要同时压“导入链路”和“查询链路”。导入链路关注 embedding 批大小、索引写入速度和增量更新；查询链路关注检索、rerank、生成和质量检查。不要用单个 hello-world 问题代表真实企业流量。
...
7. **吞吐量估算实验**：运行或阅读 `examples/rag_throughput_demo.py`，调整 `answer_generation_concurrency`、reranker 延迟和质量检查延迟，观察瓶颈阶段如何变化。

8. **压测方案设计**：为企业制度问答设计一份压测方案，至少包含并发档位、问题集构成、P95/P99、错误率、召回率、引用准确率和成本指标。

9. **Flake8 自检**：确保代码通过 `flake8 project_07_enterprise_rag/` 的检查。
