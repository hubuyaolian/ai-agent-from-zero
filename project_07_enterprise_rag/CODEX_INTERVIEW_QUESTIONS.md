...
> 基于 `project_07_enterprise_rag` 项目源码生成，涵盖架构设计、文档摄取、检索策略、安全治理、LangGraph 编排、质量评估、生产化扩展和 2026 前沿扩展共 8 大维度 46 题。
>
...
## 八、2026 前沿扩展：MCP、GraphRAG 与 Agentic RAG（6 题）

### Q41. MCP 和传统 Function Calling 的边界

课程新增了 MCP 视角。请解释 MCP 解决的是 RAG 系统中的哪一层问题，为什么它不是新的检索算法？

- 追问：MCP Server / Client / Tool Descriptor 分别对应什么职责？
- 追问：如果把企业文档库、CRM、工单系统封装成 MCP Server，权限过滤和审计应该放在 MCP 层、RAG 应用层，还是统一工具网关层？
- 追问：为什么 MCP 不能替代 `AccessFilter`、PII 脱敏、引用校验和质量评估？

### Q42. MCP 接入企业 RAG 的治理风险

如果 `DocumentLoaderFactory` 或 `HybridRetriever` 未来支持从 MCP Server 拉取外部上下文，你会如何设计安全边界？

- 追问：tool filtering、secret isolation、调用审批、trace id、租户隔离分别应该如何落地？
- 追问：MCP Server 返回的资源 metadata 是否也需要走 `mask_value` 和权限过滤？
- 追问：如果同一个 MCP Server 同时暴露“读取文档”和“删除文档”工具，RAG 应用应该如何限制可见工具集？

### Q43. GraphRAG 适合解决什么问题

GraphRAG 和当前“向量检索 + BM25 + RRF + Reranker”的路线有什么本质差异？

- 追问：实体抽取、关系抽取、社区摘要、图遍历分别解决什么问题？
- 追问：哪些问题适合 GraphRAG，例如跨部门审批链路、供应商风险传导、制度冲突分析？哪些普通制度问答不适合上 GraphRAG？
- 追问：GraphRAG 引入后，实体级 ACL、关系证据引用和图谱增量更新会带来哪些新治理问题？

### Q44. GraphRAG 与混合检索如何融合

如果在 `retrieval/` 下新增 graph retriever，你会如何把图检索结果和向量/BM25 结果合并？

- 追问：GraphRAG 的返回结果可能是实体、关系路径或社区摘要，如何转换为可引用的 evidence chunk？
- 追问：是否还能直接用 RRF？图路径分数、社区摘要质量和 chunk 排名如何归一化？
- 追问：引用校验应该引用原始文档片段、图谱关系，还是二者都要给出？

### Q45. Agentic RAG 的受控检索循环

课程新增 `examples/agentic_rag_demo.py`，它演示了查询拆解、多轮检索、证据不足拒答和最大步数约束。请说明这个示例和当前固定 pipeline 的区别。

- 追问：为什么复杂问题需要拆成多个子问题检索？
- 追问：`max_steps`、`missing_evidence`、`trace` 分别对应生产系统里的哪些治理边界？
- 追问：如果 Agent 可以自主选择向量检索、BM25、SQL、图检索和 MCP 工具，如何防止成本、延迟和工具调用失控？

### Q46. Agentic RAG 什么时候不该用

请举例说明什么时候不应该把传统 RAG 升级成 Agentic RAG。

- 追问：单文档事实查询、固定制度问答、低延迟客服场景为什么可能更适合可控 pipeline？
- 追问：Agentic RAG 带来的多步检索是否会放大 prompt 注入、权限泄漏和审计复杂度？
- 追问：你会用哪些指标判断 Agentic RAG 的收益超过成本？例如复杂问题成功率、平均检索步数、拒答准确率、token 成本和人工介入率。
...
- 能解释 MCP、GraphRAG、Agentic RAG 各自解决的层次，并能说明它们不能替代权限、审计、引用和评估闭环
...
- 把 MCP、GraphRAG、Agentic RAG 当成“新名词替换旧 RAG”，说不清接入位置、成本边界和治理要求
