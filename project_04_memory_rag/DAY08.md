...
> [!NOTE]
> ### 💡 阶段四（Day 8-10）技术栈选择说明
> 在 Day 7 中，我们学习了使用强大的 LangGraph 来编排工具 Agent。但在接下来的第四阶段（Day 8-10）中，我们会暂时“退回”到手写 `while True` 的命令交互循环。
> **这不是技术栈的倒退，而是为了更纯粹地解耦核心概念**。
>
> 记忆管理和 RAG（检索增强生成）涉及复杂的 SQL 数据存取、文本向量化、数据库语义索引等底层机制。如果我们直接把这些底层存取逻辑混在 LangGraph 的状态流转中，复杂的图节点编排会大大分散学习者的精力。
>
> 因此，在本阶段，我们以最直观的“白盒”命令循环来解构记忆与 RAG 的流转细节。当你把这里的“数据存取”、“记忆提炼” 和 “向量搜索” 彻底掌握后，你完全可以将它们包装为一个个 Tool 动作，并重新装配进 Day 7 的 LangGraph 编排结构中。

---

## 第一部分：短期记忆深化复习

### 1. 概念辨析：会话历史持久化 vs. 结构化长期记忆

在 Day 4 的课后练习中，我们要求大家尝试“用 json 或 sqlite3 将会话历史持久化”；而在 Day 8，我们又提出了“SQLite 长期记忆”。这两者在工程实践中有着截然不同的定位和目的：

- **会话历史持久化 (Session Message Persistence)**
  - **存什么**：全量的聊天消息历史对象（每一个 `HumanMessage`、`AIMessage`、`SystemMessage`）。
  - **目的**：还原对话现场。当用户关闭网页并重新打开时，能够重新渲染出之前的“聊天气泡”和聊天记录，并保留完整的上下文供模型阅读。
  - **局限**：如果仅是简单地把所有聊天记录重新发给大模型，随着聊天的不断进行，依旧会面临 **Token 膨胀与窗口溢出** 的致命瓶颈。

- **结构化长期记忆 (Structured Long-term Memory)**
  - **存什么**：从大量的聊天会话中，由大模型分析提炼出的**结构化事实 (Fact) 与偏好 (Preference)**。例如 `{"key": "fav_language", "value": "Python"}`。
  - **目的**：实现大跨度的个性化交互，并从根本上节约 Token。
  - **机制**：在对话结束后，通过专门的“提炼 Prompt”将全量消息提炼出几个精简的 KV 键值对保存到数据库；在下一轮对话开始时，不再需要把几万字的聊天记录全部发给模型，而只需要从数据库中读取这几个 KV 键值对（仅几十个字），动态拼入 `SystemMessage`。
